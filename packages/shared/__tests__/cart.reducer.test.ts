...
import { addToCart, removeFromCart, calcTotal, type CartLine, catalog } from '..'
...
describe('cart reducer pure functions', () => {
  ...
  it('increments qty when product exists', () => {
    const start: CartLine[] = [{ productId: 'apl', qty: 1 }]
  ...
  it('decrements qty when above one', () => {
    const start: CartLine[] = [{ productId: 'apl', qty: 3 }]
    const next = removeFromCart(start, 'apl')
    expect(next).toEqual([{ productId: 'apl', qty: 2 }])
  })
  ...
  it('calculates total correctly', () => {
    ...
    const cart: CartLine[] = [{ productId: 'apl', qty: 3 }]
